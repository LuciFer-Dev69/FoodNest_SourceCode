...
function LoginPage() {
  ...
  const {
    loading, fieldErrors, pendingUserId, pendingCode,
    handleLogin, handleRegister, handleVerify2FA, reset2FA,
  } = useAuthController();
        ...
        <AnimatePresence mode="wait">
          {pendingUserId ? (
            ...
            >
              ...
              {pendingCode && (
                <div data-testid="2fa-code" className="mt-4 rounded-2xl bg-primary/10 px-4 py-3 text-center">
                  <p className="text-xs text-muted-foreground">Your verification code (testing):</p>
                  <p className="text-2xl font-bold tracking-widest text-primary">{pendingCode}</p>
                </div>
              )}

              <form className="mt-6 space-y-4" onSubmit={handleVerify2FA} noValidate>
                <label className="block">
                  ...
                  <span className={`flex items-center gap-2 rounded-2xl border bg-background/70 px-3 py-2.5 focus-within:ring-2 focus-within:ring-primary/40 ${
                    fieldErrors.code ? "border-red-400" : "border-border"
                  }`}>
                    <ShieldCheck className={`h-4 w-4 ${fieldErrors.code ? "text-red-400" : "text-muted-foreground"}`} />
                    <input
                      name="code"
                      placeholder="000000"
                  ...
                  {fieldErrors.code && <p className="mt-1.5 text-xs text-red-500">{fieldErrors.code}</p>}
                </label>
                <button
                  type="submit"
                  disabled={loading}
                ...
                >
                  {loading ? "Verifying..." : "Verify & Complete"} <ArrowRight className="h-4 w-4" />
                </button>
              </form>

              <div className="mt-6 border-t border-border/60 pt-4 text-center text-sm text-muted-foreground">
                <p>
                  <button onClick={reset2FA} className="font-semibold text-primary hover:underline">Start over</button>
                </p>
              </div>
            </motion.div>
          ) : (
